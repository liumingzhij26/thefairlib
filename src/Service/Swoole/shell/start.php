...
if ($config['server']['type'] == 'http') {  //
    $server = new \TheFairLib\Service\Swoole\Network\HttpServer();
} elseif ($config['server']['type'] == 'tcp') {
    $server = new \TheFairLib\Service\Swoole\Network\TcpServer();
} elseif ($config['server']['type'] == 'udp') {
    $server = new  \TheFairLib\Service\Swoole\Network\UdpServer();
}
